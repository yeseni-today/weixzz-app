 ...
 * @since 2014-03-03
 */
public class StatusesWrapAPI  {

    private static StatusesAPI sStatusesAPI;
    private static Context sContext;
    private static StatusesWrapAPI sStatusesWrapAPI;

    private StatusesWrapAPI(){
        sStatusesAPI = APIManger.getStatusesAPI();
        sContext = WeiXzzApplication.getContext();
    }

    public static StatusesWrapAPI getInstance(){
        if (sStatusesWrapAPI==null){
            sStatusesWrapAPI = new StatusesWrapAPI();
        }
        return sStatusesWrapAPI;
    }
    ...
    public void mentions(long since_id, long max_id, int count, int page, int authorType, int sourceType,
                          int filterType, boolean trim_user, final StatusAPIRequestListener listener) {

        RequestListener requestListener = new RequestListener() {
            @Override
            public void onComplete(String s) {
               listener.onComplete(Util.handleJSONStringData(Util.JSONTYPE_MENTION_ME_STATUS, s));
            }
            @Override
    ...
    public void friendsTimeline(long since_id, long max_id, int count, int page, boolean base_app, int featureType,
                             boolean trim_user, final StatusAPIRequestListener listener) {
        RequestListener requestListener = new RequestListener() {
            @Override
            public void onComplete(String s) {
                listener.onComplete(Util.handleJSONStringData(Util.JSONTYPE_TIMELINE_STATUS, s));
            }
            @Override
            public void onWeiboException(WeiboException e) {
                listener.onError(e);
    ...
    public interface StatusAPIRequestListener{
        void onComplete(ArrayList<Status> statusArrayList);
